  ...
  static GoRouter router = GoRouter(
    routes: [
      ...
      GoRoute(
        path: KTrackingview,
        builder: (context, state) {
          return Trackingview(bCi: state.extra as BCi,);
        },
      ),
      GoRoute(
        path: KSettings,
      ...
      GoRoute(
        ...
        builder: (context, state) =>MultiBlocProvider(
          providers: [
           BlocProvider(
              create: (context) =>GetBcisCubit(Homerepoimpl(Api_services(dio: Dio())))..getdata(BlocProvider.of<AuthCubit>(context).user!.token , BlocProvider.of<AuthCubit>(context).user!.id.toString())
            ),
             BlocProvider(
              create: (context) =>AddproductCubit()
            ),
             BlocProvider(
              create: (context) =>GetproductsCubit(homerepoimpl: Homerepoimpl(Api_services(dio: Dio())))..getdata(BlocProvider.of<AuthCubit>(context).user!.token  , null)
            ),
            BlocProvider(
              create: (context) =>GetchapitreCubit(homerepoimpl: Homerepoimpl(Api_services(dio: Dio())))..getdata(BlocProvider.of<AuthCubit>(context).user!.token ),
            ),BlocProvider(
              create: (context) => AddbciCubit(Homerepoimpl(Api_services(dio: Dio())))
            ),
            BlocProvider(
              create: (context) => GetarticlesCubit(homerepoimpl: Homerepoimpl(Api_services(dio: Dio())))
            ),
          ],
          child: Home_view(),
        )
